...
async function visitOneSite(site) {
  ...
  try {
    const siteVisit = await navigation.visitPageAndInterceptURLs(
      site.domainAddress
    );
    ...
    if (siteVisit.canvasFingerprintingDetected) {
      await database.setSiteCanvasFingerprinting(
        workerData.popularSiteCollectionName,
        site
      );
    }
    ...
    for (const requestFromSite of siteVisit.requests) {
      await database.upsertTrackerToDatabse(
        workerData.trackerCollectionName,
        site,
        requestFromSite
      );
    }

    return await database.getOneSitesWithRequestsAndOwners(
      site._id,
      workerData.popularSiteCollectionName,
      workerData.trackerCollectionName,
      workerData.siteOwnersCollectionName
    );
  } catch (error) {
    console.error(error);
  }
...
parentPort.on("message", (message) => {
  console.log(
    `[${moment().format("DD.MM.YYYY HH:MM:SS")}] Site Visitor: '${
